...
export class ShopperProfileComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.user.cin) {
            ...
            if (this.user.cin.filename) {
                this.cinURL = environment.apiURL + '/' + this.user.cin.filename
            } else {
                this.cinURL = 'https://via.placeholder.com/300?text=No CIN provided'
            }
            console.log('cin : ', this.cinURL)
        }
    ...
    onPageChange(page: number) {
        ...
        if (page !== this.skip) {
            this.skip = page;
            this.getDeliveries(this.limit, this.skip);
        }
    }
